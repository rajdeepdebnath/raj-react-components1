...
export function fetchToken(
  ...
  passcode: string,
  create_room = true,
  create_conversation = false//process.env.REACT_APP_DISABLE_TWILIO_CONVERSATIONS !== 'true'
) {
  ...
  //   }),
  // });
  return getTokenFromTwilio('test_user');
}
...
export function verifyPasscode(passcode: string) {
  return fetchToken('temp-name', 'temp-room', passcode, false /* create_room */, false /* create_conversation */).then(
    async res => {
      ...
      if (res.ok && jsonResponse.accessToken) {
        return { isValid: true };
      }
    }
  );
